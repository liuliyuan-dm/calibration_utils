...
public:
    ...
    vector<Vector3d> ab_m_;    // acc body measurement,这个归一化算，不考虑g的具体值
...
{
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    Quaterniond quat_e_ = Quaterniond(1.0, 0.0, 0.0, 0.0);
    Quaterniond quat_u_ = Quaterniond(1.0, 0.0, 0.0, 0.0);
};
...
public:
    ...
    //bool ReadDataFromTxt(vector<Vector3d> &omega_body_measurements_, vector<Vector3d> &acc_body_measurements_, vector<double> &timestamps);
